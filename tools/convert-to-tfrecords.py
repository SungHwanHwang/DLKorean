...
# Image csv 파일, 라벨, tf_records output
...
class TFRecordsConverter(object):
    """ 획득한 이미지 파일을 활용, Tensorflowdml 표준 입력 형식으로 변형"""
    ...
    def process_image_labels(self, labels_csv, label_file):
        ...
        for row in labels_csv:
            file, label = row.strip().split(',')    # ','로 구분되어 있는 csv 파일
            images.append(file)
    ...
    def write_tfrecords_file(self, output_path, indices):
        ...
        for i in indices:
            ...
            # Key-Value 형식으로 저장된 Data Format
            #   1. Raw Image Data에 대한 ByteList
            #   2. 라벨의 인덱스를 포함한 Int64List
    ...
    def convert(self):
        """
        - TFRecords로의 변환 함수
        - TrainingSet과 TestingSet 두가지로 변환
        """
        # 전체
        ...
        for i in range(1, self.num_shards_train):
            shard_path = os.path.join(self.output_dir, 'train-{}.tfrecords'.format(str(i)))   # tfrecords-output 디렉토리
        ...
        print('Processed {} total images...'.format(self.counter))
